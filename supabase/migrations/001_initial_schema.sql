...
--
-- ⚠️  DEV RESET ONLY — the DROP block at the top destroys all data.
--     For production use proper incremental migrations.
--     Safe to re-run only on a clean / dev Supabase project.
-- ============================================================
...
CREATE TABLE bankroll_transactions (
  ...
  balance_after numeric NOT NULL, -- always snapshot running balance
  metadata      jsonb,
  created_at    timestamptz DEFAULT now()
);
...
-- ─── CROSS-USER VALIDATION TRIGGERS ─────────────────────────

-- bets.bankroll_id must belong to same user
...
BEGIN
  IF NEW.bankroll_id IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1 FROM bankrolls WHERE id = NEW.bankroll_id AND user_id = NEW.user_id
    ) THEN
...
-- bankroll_transactions: both bankroll_id and bet_id must belong to same user
...
-- bet_legs.decision_id must belong to same user as parent bet
...
BEGIN
  IF NEW.decision_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      JOIN bets b ON b.id = NEW.bet_id
...
-- Security: user identity derived from auth.uid() — NOT from caller argument.
-- All four writes happen in one transaction; partial failure = full rollback.
CREATE OR REPLACE FUNCTION create_quick_bet(
  p_bankroll_id   uuid,       -- which bankroll to use (or null = default)
  p_event_name    text,
  p_sport         text,
  p_market_type   text,
  p_selection     text,
...
AS $$
DECLARE
  v_user_id     uuid := auth.uid();  -- never trust caller for identity
  v_decision_id uuid;
  v_bet_id      uuid;
...
BEGIN
  ...
  -- Resolve bankroll: validate ownership, fall back to default, create if needed
  IF p_bankroll_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    SELECT id INTO p_bankroll_id
    FROM bankrolls WHERE user_id = v_user_id AND is_default = true
    LIMIT 1;

    IF p_bankroll_id IS NULL THEN
      -- Safety net: create default bankroll if missing
      INSERT INTO bankrolls (user_id, name, is_default)
      VALUES (v_user_id, 'Main', true)
      RETURNING id INTO p_bankroll_id;
    END IF;
  END IF;
  ...
  -- 3. BetLeg (decision created by same user — safe)
...
-- bet_legs: via parent bet ownership + cross-user trigger above
