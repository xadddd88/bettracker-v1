-- Migration 028: make tennis step confirmation database-authoritative.
--
-- Migration 027 accepted derived financial values from the application boundary.
-- This replacement removes those parameters from the callable contract and derives
-- loss, recommended stake, target snapshot, and projected series result while the
-- series row is locked. The feature remains server-only and default OFF.
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.tennis_series_commands) THEN
    RAISE EXCEPTION 'authoritative_upgrade_refused: tennis_series_commands is not empty';
  END IF;
END
$$;

REVOKE ALL ON FUNCTION public.tennis_confirm_step(
  uuid,uuid,uuid,integer,integer,integer,numeric,numeric,numeric,numeric,numeric,numeric,numeric
...
BEGIN
  ...
  -- All conversion is exact because schema checks cap money at two decimal
  -- places and odds at three. Integer division below implements the same
  -- ceil/floor policy as lib/tennis/series-math.ts.
  ...
  -- Multiplication by the scale-2 literal keeps the exact cent precision
  -- accepted by migration 026's scale(value) <= 2 storage checks.
  v_recommended_stake := v_recommended_minor::numeric * 0.01;
  v_projected_series_result := v_projected_minor::numeric * 0.01;

  -- bankroll_limit is the total isolated series bankroll, so realised loss
  -- reduces remaining capacity. Both the recommendation and the actual stake
  -- must fit; neither is silently reduced.
  ...
  INSERT INTO public.tennis_series_steps (
    ...
    'confirmed', v_loss_before, v_series.target_profit, v_projected_series_result
  )
...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION 'legacy client-derived confirm signature still exists';
  END IF;
