...
const webAi = source('app/(app)/ai/page.tsx')
...
const riskRoute = source('app/api/risk/evaluate/route.ts')
const riskEvaluator = source('components/risk/RiskEvaluator.tsx')
...
assert.match(webDecision, /canCheckRisk=\{showPricing/)
...
assert.doesNotMatch(webAi, /place_bet_from_decision/)
assert.doesNotMatch(riskRoute, /recommended_max_stake|recommendedMaxStake|RECOMMENDED_MAX_PCT/)
assert.doesNotMatch(riskEvaluator, /recommended_max_stake|Suggested max|Place Bet|Place anyway|onConfirm/)
