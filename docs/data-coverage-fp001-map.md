...
4. Keep M1.2.e.2 SportMonks canonical fixture mapping scope DONE and treat the missing exact/high SportMonks provider link as a blocker for canonical-linked enrichment.
5. Approve read-only dry-run scope before any provider call; shape-only/unbound dry-runs remain unable to write, attach to `canonical_fixture_id`, or unlock downstream usage.
6. Run canonical-linked enrichment read-only dry-runs only under explicit request budgets and only after exact/high provider-link prerequisites are met.
7. Design enrichment storage/write gates separately.
8. Add Analyst-layer per-leg input contracts only after provider-backed data coverage is validated.
