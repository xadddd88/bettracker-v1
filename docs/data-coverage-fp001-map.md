...
| Team news / lineups | API-Football lineups endpoint or another licensed football enrichment provider | Existing `football_enrichment` table requires schema review before reuse; future curated enrichment storage may be needed for lineups, starting elevens, formations, player availability, source timestamp | M1.2.e football enrichment design, then endpoint evidence and read-only dry-run | Design in review. Implementation not started. | Confirm endpoint evidence/cost, review schema, define freshness, run read-only dry-runs, add write gates, validate freshness and coverage. Does not belong to current M1.3 bookmaker/mapping path. |
| Injuries / suspensions | API-Football injuries/sidelined endpoints if available on current plan; alternative licensed football enrichment provider if needed | Existing `football_enrichment` table requires schema review before reuse; future curated enrichment storage may be needed for injuries, suspensions, availability status, source timestamp, confidence/freshness metadata | M1.2.e football enrichment design, then endpoint evidence and read-only dry-run | Design in review. Implementation not started. | Confirm provider coverage, plan access, request cost, response shape, player/team mapping, freshness, legal/licensed usage, write gates, and trust validation. |
...
2. Design football enrichment separately for lineups, injuries, suspensions, and event-state freshness.
3. Confirm football enrichment endpoint evidence before any provider call.
4. Run enrichment read-only dry-runs under explicit request budgets.
5. Design enrichment storage/write gates separately.
6. Add Analyst-layer per-leg input contracts only after provider-backed data coverage is validated.
...
M1.2.e football enrichment is now a separate design track only. It does not start provider calls, migrations, writes, Scout usage, Analyst usage, UI usage, Place Bet, or betting signals.
