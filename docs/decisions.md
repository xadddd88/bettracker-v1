...
**Status:** ACTIVE / PHASE A APPLIED, CATALOG VERIFIED, AUTHENTICATED SMOKE VERIFIED; PHASE B HOLD. Migration 024 was applied to production on 2026-07-16 as `20260716142736_create_tracked_bet_024`, and the exact catalog contract was verified read-only. Phase B (UI/API adoption: unified Single/Express form, Scanner → editable legs → Bet, mobile daily tracker flow) remains HOLD under Decision #060 and requires separate CPO approval — it does not consume a new decision number. Highest-numbered CLOSED decision remains #059.
...
**Post-smoke holds:** UI/API changes 0; `create_quick_bet` unchanged; Phase B HOLD pending separate CPO approval. Decision #056 runtime remains NOT APPROVED / NOT RUN. Results ingestion and automated settlement remain HOLD. Decision #050 SMTP round-trip remains PENDING. CSP Phase B remains NOT APPROVED. FP-001 remains ACTIVE.
