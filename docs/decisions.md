...
**Status:** EXECUTED / VERIFIED / CLOSED 2026-07-26. Migration 021 applied + verified (enum += invited, `invited_at`, existing rows intact) → PR #133 merged (production `60cb28c`) → live routes verified server-side. Founder confirmed a real production invite email → action link → set-password → dashboard round-trip; read-only verification confirmed the row reached `used` with `used_by_user_id` and a matching Auth user. A unique non-allowlisted production request returned the neutral response with 0 `beta_access` rows, 0 `auth.users`, and no invite-send log. Supabase Invite uses `{{ .ConfirmationURL }}` and "Allow new users to sign up" is OFF. Execution record: `docs/registration-invite-flow-execution-record-050.md`. Next free decision number at closure remains historical #051; current next unreserved is #068.
...
**Execution verification:** Completed 2026-07-26. The founder confirmed the controlled mailbox round-trip (email → link → password → dashboard), and read-only production checks verified the matching `approved → invited → used` lifecycle. The negative non-allowlisted request produced the neutral response without creating `beta_access` or Auth-user state and without an invite-send log. The Invite template uses `{{ .ConfirmationURL }}`, the runtime redirect remains `/auth/callback?next=/auth/set-password`, and "Allow new users to sign up" is OFF. Custom SMTP is not configured; the verified Supabase email-service delivery is sufficient for #050, while custom SMTP scale/readiness remains a separate follow-up.
...
## Decision #060 — Founder-First Coupon-to-Tracker
**Date:** 2026-07-15
**Proposed by:** CPO + Founder
...
**Phase A contract (implemented):** identity `auth.uid()` only; default bankroll only (no caller-supplied bankroll id); inputs legs jsonb (1–20), total_odds, stake, bookmaker, notes, source `manual|scanner`, REQUIRED UUID idempotency_key canonicalized to lowercase; per-leg fail-closed validation (only `sport|event_name|market_type|selection|odds` keys, `selection` nullable — absent/JSON null accepted; canonical sports `soccer|tennis|basketball|ice_hockey|cs2|mma|other`; odds JSON number in (1,10000]); canonical values (trimmed strings, NULL-collapsed selection, `trim_scale()` odds/stake/total_odds) used for BOTH the hash and the inserted rows, making `2`/`2.0`/`2.00` equivalent; 1 leg → single with total_odds from the normalized leg, 2–20 → parlay with required normalized total_odds; leg order preserved via new additive nullable `bet_legs.leg_index` with CHECK 1–20 + partial UNIQUE (bet_id, leg_index); NO Decision rows; bankroll `FOR UPDATE` + no-overdraft; bet + legs + balance + stake transaction atomic; strict payload-bound idempotency via `bankroll_transactions.idempotency_key` (SHA-256 request hash over the normalized payload; replay only when stored tx has type='stake' + non-null bet_id + identical hash → original bet_id, replayed=true, no second deduction; any other key reuse incl. cross-function → `Idempotency conflict`, zero writes); transaction metadata ONLY `{request_hash, source, leg_count}` — no rawText/statusText/scoreText/screenshots/event names; empty pinned `search_path` + schema-qualified `public.*` objects; REVOKE from PUBLIC/anon, GRANT only authenticated/service_role; zero new direct DML grants and zero RLS changes; exact-signature read-only catalog checks, atomic fail-closed rollback, and disposable PostgreSQL 17 verifier included. The production authenticated smoke was a separate CPO-authorized execution.

**Production Phase A checkpoint (2026-07-16):** migration version `20260716142736_create_tracked_bet_024` is applied, and the exact catalog contract passed read-only verification. The authenticated smoke used a dedicated non-login synthetic account with a seed deposit of 100. Two `create_tracked_bet` calls exercised one initial write and one exact semantic replay: the first response returned `replayed=false` and balance 90; the replay returned `replayed=true`, the same `bet_id`, and balance 90.

**Smoke integrity and cleanup:** before cleanup, the account had 1 bet, 1 leg, 2 transactions, 1 stake transaction, and 0 decisions. The replay produced zero additional writes. Canonical normalized bet/leg values and the metadata allowlist were verified. The account and all related rows were deleted. An independent post-transaction cleanup check confirmed users/profiles/bankrolls/transactions/bets/legs/decisions = 0, and all `bet_legs` rows with non-null `leg_index` = 0.

**Tests:** financial-safety +6 migration-024 guard tests (31/31) — incl. sha256-not-md5, normalized-value hashing/`trim_scale`, strict replay triple-check, nullable selection, CHECK+UNIQUE order integrity, empty-search-path hardening, atomic rollback/exact-signature catalog checks, and the disposable PostgreSQL 17 runtime verifier; domain-write-boundaries +1 additive-only boundary test (14/14); no regressions across provider-safety 97/97, analysis-quality-gate 26/26, auth-invite 16/16, rate-limit 12/12, csp-security 18/18; tsc + lint clean.

**Phase B implementation (2026-07-16, approved by CPO):** unified Single/Express tracker form at `/bets/new` — 1 leg = Single, 2–20 legs = Express with add/edit/remove and preserved order (array order = `leg_index` 1..n); per-leg `sport/event_name/market_type/selection/odds` plus `stake`, express `total_odds`, `bookmaker`, `notes`; mobile-first (320/375 px single column, full-width controls) with no desktop regressions; the calculated express total is a UI preview only — the RPC stays the financial authority. The existing `/api/ai/scanner` OCR flow feeds an allowlisted camelCase→snake_case adapter (`lib/bets/tracked-bet.ts`): only the five contract fields reach editable legs; `rawText`/`statusText`/`scoreText`/`isLive`/`periodOrPhase` never leave the scan handler; unreadable leg odds stay empty (the coupon total is never copied onto legs); scanned submissions use `source='scanner'`, manual entry `source='manual'`. Writes go ONLY through new `POST /api/bets/tracked`: authenticated session (401 before the limiter and the RPC), Decision #052 rate limit `RATE_LIMITS.trackedBet` (fail-closed 503, 429 + Retry-After), strict shared zod schema (`.strict()` request + legs, RPC mirror incl. express-total requirement), `create_tracked_bet` called as the AUTHENTICATED user (cookie client — service_role never appears in the user flow), sanitized error mapping (422 Insufficient balance / 404 Bankroll not found / 409 Request conflict / 422 validation / 500 Transaction failed — no raw DB text). Client idempotency is a PURE state machine in `lib/bets/tracked-bet.ts` (payload fingerprint + UUID + status `ready|in_flight|conflict`, injected UUID generator) that the form wires without any second implementation: one UUID per payload snapshot; double clicks blocked while in_flight; exact retry after network/429/503/5xx reuses the same key+payload (RPC replays, no second deduction); success clears the intent and a later submit mints a fresh UUID; a 409 NEVER clears or rotates the key and never auto-retries — the unchanged intent is blocked client-side with the fixed error `Request conflict`, and only a deliberate payload change starts a new intent with a new UUID. Success and replay both open the created bet at `/bets/<bet_id>` with `router.refresh()`. `/bets` and `/bets/[id]` order embedded legs by `leg_index`. `create_quick_bet` remains defined and unchanged with zero remaining UI callers (deprecation is a separate decision).

**Phase B tests:** financial-safety +22 (53/53) — 11 BEHAVIORAL intent-machine unit tests importing the compiled helper with a deterministic injected UUID generator (single-UUID first submit, in-flight block, retryable failures keep UUID+snapshot, exact-retry key reuse, 409 conflict keeps the key, unchanged post-409 block without a new UUID, payload change → new intent/UUID, success clears, post-success fresh UUID, determinism, fingerprint stability) plus 11 route/API tests — 401 before limiter/RPC; 429 + Retry-After keyed `tracked-bet:<user>`; 503 fail-closed; strict-schema fail-closed (unknown request/leg fields, 21 legs, express without total, negative stake; 20 legs accepted) with zero RPC calls; manual Single mapping; scanner Express mapping (order + exactly five contract keys per leg); nullable selection; sanitized 422/404/409 + generic 500 with leak assertions; replay passthrough; supplementary wiring assertions (route ONLY at `app/api/bets/tracked/route.ts`, `/api/bets/tracked`-only submit, success/replay → `/bets/<bet_id>`, form uses the shared intent machine with one injected UUID generator and no local lifecycle, no service_role, no direct table access); leg ordering + `create_quick_bet` regression. rate-limit 12/12 (trackedBet config + route sweep). No regressions: domain-write-boundaries 14/14, provider-safety 97/97, analysis-quality-gate 26/26, auth-invite 16/16, csp-security 18/18, tsc + lint clean.

**Production Phase B execution record (2026-07-16):** PR #159 merged as `1926d9a82759cd1e4e97378ca15addf010c0bf28` and the production deployment reached READY. From 2026-07-16T18:24:10Z to 18:24:22Z, a temporary authenticated synthetic user issued exactly one `POST /api/bets/tracked`. It returned HTTP 200 with `replayed=false` for a manual Single (stake 1, odds 2), changed the seeded balance 100→99, and created exactly 1 bet, 1 leg with `leg_index=1`, 1 stake transaction, and 0 Decision rows. Stake metadata contained exactly `leg_count`, `request_hash`, and `source`.

**Phase B smoke integrity and cleanup:** one earlier password-auth attempt failed before any tracked-bet POST because the temporary Auth fixture required token-field normalization; it caused no bet or stake write and did not consume the one authorized tracked-bet call. Global sign-out left 0 active sessions. Deleting the temporary user cascade-cleaned users, identities, sessions, profiles, bankrolls, transactions, bets, legs, and decisions to zero. Provider calls were 0. This was an authenticated production API-level smoke of the cookie-session route and RPC; authenticated browser/UI automation was not performed.
...
**Date:** 2026-07-17
**Proposed by:** CPO + Founder
**Approved by:** CPO (Phase A read-only report ACCEPTED; Phase A1 verdict IMPLEMENTATION CONTINUE); Founder (Phase A2 IMPLEMENT and merge, 2026-07-27)
**Status:** EXECUTED / VERIFIED / CLOSED — Phase A1 merged via PR #162 as `a6d4ebbefcf49af71729c64cd33886d0592cf1fd` and deployed READY. Phase A2 exact head `b14168a246cfb76f0d566cd795eed758805f4e48` passed all 10 CI jobs, including Hermetic Web acceptance at 320/375/1280 px, and PR #235 merged it as `2fb83250cc648d990f844bbb8edc73ae81dc5a17`; the automatic Vercel production deployment reported success.

**Decision:** Accept the founder's daily flow (sign in → scan/type a coupon at `/bets/new` → review → Save → `/bets/<bet_id>` → totals on `/bets` and the dashboard) as reliable for everyday use. Phase A produced a read-only starting-point assessment. Phase A1 closed the three P0/P1 correctness defects that assessment found. Founder authorization on 2026-07-27 lifted the Phase A1-only deferral for Phase A2: implement the hermetic local E2E harness (Playwright + Supabase stub at 320/375/1280 px) under #061 without consuming #068.
...
**Tests:** Phase A1 financial-safety +12 (65/65), in two distinct classes. (a) 5 BEHAVIORAL compiled-adapter tests — they execute the compiled `scannerDataToDrafts` against synthetic coupons and assert real return values (20-leg full import in coupon order; 21-leg wholesale refusal via deepEqual; raw-count overflow under empty-name filtering; full-replacement empty strings; single-leg/legacy fallback). (b) 7 STATIC source/wiring assertions — regex/index checks over the page source text, not executed behavior (ok-check ordering before every state write + fixed message + no truncation path; submit-gate ordering before `.safeParse(`/`beginSubmit(`/fetch with 0 requests and 0 UUIDs while blocked; exact unlock surface — one arm site, two unlock sites, 7 payload-edit sites through `markManualEdit`, manual unlock switches source to manual, no `clearError` bypass; unconditional stake/bookmaker + scanner-never-writes-notes; fieldset/aria-busy/button lock; 3 synchronous entry-point guards + no AbortController + finally release; route/RPC/migration-024/intent-machine surface untouched). Phase A2 adds the running-DOM proof at 320/375/1280 px: delayed 21-leg scan locks the whole form; refused overflow Save emits 0 tracked requests and 0 UUIDs; one manual edit clears the block and submits `source='manual'`; delayed Save preserves the complete busy lock and one-request/one-UUID invariant; synthetic success is rendered on detail, `/bets`, and dashboard. Acceptance was satisfied on exact head `b14168a246cfb76f0d566cd795eed758805f4e48`: Preview Tests run #389 completed 10/10 jobs successfully, including `Hermetic Web acceptance` and the existing regression suite.

**Merge and deployment checkpoint:** PR #162 accepted head `9fd1441f70bb782d51f444a0be85a405c8123ff0` and merged Phase A1 as `a6d4ebbefcf49af71729c64cd33886d0592cf1fd`; all 7 resulting blob hashes match the accepted head. Production deployment `dpl_CkCEBy243hsJDcymMgZycZnby8Pw` reached READY with 0 post-deployment runtime errors. No production smoke or scanner/API/Supabase runtime call or write was performed for Phase A1. Browser-level verification was not performed in Phase A1. Phase A2 exact head `b14168a246cfb76f0d566cd795eed758805f4e48` passed Preview Tests run #389 with 10/10 jobs successful and merged via PR #235 as `2fb83250cc648d990f844bbb8edc73ae81dc5a17`; the automatic Vercel production deployment reported success. Phase A2 performed no production smoke and no real scanner, tracked-bet, Supabase, provider, AI, settlement, or financial call/write.

**Boundaries:** 0 production/Supabase/provider calls; 0 migrations, RPC, schema, API-route, env, or production-service changes; `create_tracked_bet`, `create_quick_bet`, and `POST /api/bets/tracked` unchanged. Runtime scope is limited to `app/(app)/bets/page.tsx`, where the existing date inside the populated-list `<dl>` becomes a visually neutral, accessible `<dt>/<dd>` group with no behavior or financial/data-flow change. Phase A2 intercepts scanner and tracked-bet responses in the browser, exposes the resulting synthetic record only through a localhost read-only Supabase stub, and fails closed on all non-loopback traffic and non-GET/HEAD Supabase requests. Settlement/results HOLD; Decision #056 runtime NOT APPROVED; FP-001 ACTIVE. Decision #061 is CLOSED after Phase A2 passed trusted CI and the separate merge gate; closure grants no new runtime authority.

**Numbering:** Decision #061 is occupied and CLOSED; Decision #062 is occupied and ACTIVE; #063, #064, #065, and #066 are occupied and CLOSED; #067 is occupied and CLOSED; next unreserved decision #068.
...
**Status:** WEB ROLLOUT CLOSED / PRODUCTION DEPLOYED — verified stages PR A–J, reconciled in PR #202, then production-hardened through #203 → #224. Mobile release remains unauthorized.
...
WebSocket preblocking. PR #202 reconciled that exact reviewed source. Follow-up
PRs #203 → #224 stabilized hydration, money formatting, AI report contrast, AI
language/stale-event gates, Russian authenticated UX, production smoke selectors,
and Russian auth/set-password screens. The final production checkpoint is
`eb51d91`.
...
**Web-first and non-authorization:** Web was released first. Mobile source in the
integration diff is not approval for EAS Build/Update, Android/iPhone builds,
beta distribution, or app publication. Supabase/DB writes or migrations, provider
runtime calls, Event First activation, and any new financial or settlement
runtime remain unauthorized. CSP enforcement remains separately governed by
Decision #054 Phase B and is not approved here.
...
three P3 items were closed before production release and remained guarded by
...
**Proposed by:** CPO
**Approved by:** Founder (Draft PR scope only)
**Status:** EXECUTED / VERIFIED / CLOSED — PR #232 merged as `8ce79df4444c366b07a3585fde3de8554f431b4a`; migration 030 applied once as `20260727093233_odds_snapshots_public_security_invoker_030` and verified.
...
**Initial Draft non-authorization:** migration apply, production DB writes, odds ingestion, provider calls, application callers, env changes, Vercel actions, merge, and production deployment were outside the implementation Draft approval.
...
**Intentional exceptions:** Nine authenticated `SECURITY DEFINER` RPCs remain part of the application contract and retain `auth.uid()`-scoped or otherwise reviewed boundaries. Their Advisor warnings remain documented exceptions rather than being hidden by unsafe conversion to `SECURITY INVOKER`.
