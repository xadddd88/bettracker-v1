...
**Status:** ACTIVE: APPLIED / CATALOG VERIFIED; authenticated smoke pending. Migration 024 was applied to production on 2026-07-16 as `20260716142736_create_tracked_bet_024`, and the exact catalog contract was verified read-only. Runtime bet creation through the new RPC has not started. Phase B (UI/API adoption: unified Single/Express form, Scanner → editable legs → Bet, mobile daily tracker flow) remains HOLD under Decision #060 and requires separate CPO approval after an approved authenticated smoke — it does not consume a new decision number. Highest-numbered executed decision remains #059.
...
**Post-apply holds:** authenticated smoke NOT APPROVED / NOT RUN; `create_tracked_bet` RPC runtime calls 0; UI/API changes 0; `create_quick_bet` unchanged; Phase B HOLD. Decision #056 runtime remains NOT APPROVED / NOT RUN. Results ingestion and automated settlement remain HOLD. Decision #050 SMTP round-trip remains PENDING. CSP Phase B remains NOT APPROVED. FP-001 remains ACTIVE.
