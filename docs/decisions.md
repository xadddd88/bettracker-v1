...
**Boundaries:** 0 production/Supabase/provider calls; 0 migrations, RPC, schema, runtime-code, API-route, env, or production changes; `create_tracked_bet`, `create_quick_bet`, and `POST /api/bets/tracked` unchanged. Phase A2 intercepts scanner and tracked-bet responses in the browser, exposes the resulting synthetic record only through a localhost read-only Supabase stub, and fails closed on all non-loopback traffic and non-GET/HEAD Supabase requests. Settlement/results HOLD; Decision #056 runtime NOT APPROVED; FP-001 ACTIVE. Decision #061 remains ACTIVE until Phase A2 passes trusted CI and is separately approved for merge.
