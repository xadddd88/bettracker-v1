...
**Status:** ACTIVE / PHASE A IMPLEMENTATION. The Phase A PR adds additive migration 024 (review-only — NOT applied to production by this PR) with the `create_tracked_bet()` SECURITY DEFINER RPC, its emergency rollback script (`docs/decision-060-rollback.sql`), and its trust-suite guards. Runtime bet creation through the new RPC does not happen yet. Phase B (UI/API adoption: unified Single/Express form, Scanner → editable legs → Bet, mobile daily tracker flow) remains under Decision #060 with its own separate CPO approval after Phase A is applied and verified — it does not consume a new decision number. Highest-numbered executed decision remains #059.
...
**Non-use (Phase A):** production migration NOT applied by this PR; runtime create_tracked_bet calls 0; UI/API changes 0; `create_quick_bet` changes 0; provider calls 0; Supabase runtime calls/writes 0. Decision #056 runtime remains NOT APPROVED / NOT RUN. Results ingestion and automated settlement remain HOLD. Decision #050 SMTP round-trip remains PENDING. CSP Phase B remains NOT APPROVED. FP-001 remains ACTIVE.
