...
| **Current phase** | M1.2 provider-backed fixture foundation complete; M1.3 odds provider evidence in draft PR #82; Product Vision Gap / Beta v2 planning continues |
...
M1.3 Odds Snapshot Sync Design is DONE via PR #79. M1.3 Odds Endpoint Discovery & Dry-Run Plan is DONE via PR #80. M1.3 API-Football Odds Endpoint & Cost Confirmation is DONE / BLOCKED via PR #81. M1.3 odds writes, migrations, production provider odds calls, Scout, Analyst, and UI usage remain NOT STARTED.
...
| Field | Value |
|---|---|
| **Status** | DESIGN DONE via PR #79; endpoint discovery / dry-run planning DONE via PR #80; endpoint/cost confirmation DONE / BLOCKED via PR #81; provider evidence partial in draft PR #82 |
| **Implementation** | READ-ONLY PLANNER ONLY from PR #80; odds ingestion NOT STARTED |
| **Odds ingestion** | NOT STARTED |
| **Provider calls** | NOT RUN; production odds provider calls blocked until remaining cost/discovery evidence is accepted |
...
- PR #82 partially confirms `/odds`, fixture-specific request shape, pagination, `Match Winner` = bet id `1`, and sanitized response shape
- production provider odds calls remain blocked until cost per request and bookmaker/market discovery response shapes are accepted
