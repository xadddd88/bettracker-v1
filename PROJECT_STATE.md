...
| **Current phase** | M1.2 provider-backed fixture foundation complete; M1.3 read-only odds dry-run executed safely with no odds coverage and no writes; bookmaker/mapping discovery rerun after PR #98 is partial/safe and stopped on mapping pagination guard; canonical-fixture-first page-1 mapping comparison is DONE / NOT FOUND; filtered mapping support evidence is DONE / FILTERED RUNTIME BLOCKED; API-Football `/odds/mapping` filter evidence is in docs/status review; M1.3 mapping exploration is PAUSED; Product Vision Gap / Beta v2 planning continues |
...
- API-Football `/odds/mapping` filter evidence remains documentation/status only; runtime filters, page 2, and pages 2-11 remain blocked
...
M1.3 API-Football mapping filter evidence reference: `docs/sports-odds-mapping-filter-evidence-m1-3.md`
