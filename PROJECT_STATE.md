...
- Do NOT start M1.2.e football enrichment implementation; read-only dry-run scope is done, canonical-linked runtime remains blocked on an exact/high SportMonks provider link, M1.2.e.2.b SportMonks mapping discovery scope is DONE (runtime still not approved), and no runtime provider call is approved
- Do NOT start i18n
