...
export default async function BetDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="bn-page mx-auto flex w-full max-w-4xl flex-col gap-4 pb-8">
      ...
      <Link className="bn-button bn-button-secondary w-fit" href="/bets">← Ставки</Link>

      <BroadcastPanel className="p-5 sm:p-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <p className="editorial-kicker">Сохранённая запись · {isExpress ? `экспресс, плеч: ${legs.length}` : 'одиночная'}</p>
            <h1 className="mt-3 break-words font-display text-[clamp(2rem,6vw,4.5rem)] font-black leading-[0.98] tracking-[-0.05em] text-bn-text">
              {isExpress ? 'Экспресс-купон' : legs[0]?.event_name || 'Сохранённая ставка'}
            </h1>
          </div>
        ...
        <dl className="mt-7 grid grid-cols-2 gap-px overflow-hidden rounded-control border border-bn-border-subtle bg-bn-border-subtle sm:grid-cols-4">
          ...
          />
          <DataPoint label="Букмекер" value={bet.bookmaker || '—'} />
        </dl>
      </BroadcastPanel>
      ...
      <BroadcastPanel className="overflow-hidden p-0">
        <div className="flex min-h-14 items-center justify-between gap-4 border-b border-bn-border-strong px-5 py-3 sm:px-7">
          <h2 className="font-display text-xl font-black tracking-[-0.035em] text-bn-text">{isExpress ? 'Плечи купона' : 'Выбор'}</h2>
          <span className="font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-bn-quiet">Порядок: {legs.length}</span>
        </div>

        {legs.length ? (
          <ol aria-label="Плечи купона" className="divide-y divide-bn-border-strong">
            {legs.map((leg, index) => (
          ...
          <p className="p-6 text-sm text-bn-muted">Детали плеч не записаны.</p>
        )}
      </BroadcastPanel>

      <BroadcastPanel className="p-5 sm:p-7">
        <h2 className="font-display text-xl font-black tracking-[-0.035em] text-bn-text">Детали записи</h2>
        <dl className="mt-5 divide-y divide-bn-border-subtle">
          <DetailRow label="Поставлена" value={formatDateTime(bet.placed_at)} />
          {bet.settled_at ? <DetailRow label="Рассчитана" value={formatDateTime(bet.settled_at)} /> : null}
          <DetailRow label="Источник" value={bet.source || '—'} />
        </dl>
        {bet.notes ? (
          <div className="mt-5 border-t border-bn-border-strong pt-5">
            <div className="font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-bn-quiet">Заметки</div>
